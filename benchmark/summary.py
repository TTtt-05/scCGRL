"""Combine baseline outputs and compute long-format benchmark summaries."""
...
def summarize(results_root: Path = RESULTS_ROOT):
    ...
    for dataset in DATASETS:
        for method in ALL_METHODS:
            ...
            if "native_branch_output_available" in frame:
                ...
                columns = [
                    column for column in ("F1_branches", "endpoint_count", "branch_count")
                    if column in frame
                ]
                frame.loc[unavailable, columns] = float("nan")
            if "native_topology_output_available" in frame:
                ...
                columns = [
                    column for column in ("HIM_similarity", "trajectory_overall_geometric_mean")
                    if column in frame
                ]
                frame.loc[unavailable, columns] = float("nan")
    ...
    all_runs.to_csv(
        results_root / "combined" / "benchmark_all_runs.csv",
        index=False,
        encoding="utf-8-sig",
    )
    summaries, completion = [], []
    if not all_runs.empty:
        grouped = all_runs.groupby(["dataset", "method"], sort=False)
    ...
    combined.to_csv(
        results_root / "combined" / "benchmark_summary.csv",
        index=False,
        encoding="utf-8-sig",
    )
    pd.DataFrame(completion).to_csv(
        results_root / "combined" / "benchmark_completion.csv",
        index=False,
        encoding="utf-8-sig",
    )
