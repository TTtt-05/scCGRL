...
def make_figures(output_dir, run_output_root, seed=42, episodes=10000):
    ...
    # C uses RF pseudotime; E/F use trajectory pseudotime
    ...
    TARGET_LINEAGES = ['2', '3']
    EXPLICIT_COLORS = {'2': '#f9736e', '3': '#95CAE9'}

    unique_cell_types = np.unique(cell_types_arr)
    global_palette = sns.color_palette("Set2", len(unique_cell_types))
    GLOBAL_COLOR_MAP = {ctype: mpl.colors.to_hex(c) for ctype, c in zip(unique_cell_types, global_palette)}
    ...
    def extract_and_smooth_tree(paths_dict, coords, num_points=100, sigma=3, smooth=False):
        ...
        for nodes in segments:
            ...
            arc = np.concatenate(([0], np.cumsum(np.linalg.norm(np.diff(uni, axis=0), axis=1))))
            if arc[-1] > 0:
                arc /= arc[-1]
    ...
    def nw_gaussian_curve(pt, expr, sigma_pt=BIO_PT_SIGMA, grid_size=BIO_GRID_SIZE):
        ...
        smooth = np.divide(weights @ expr, denom, out=np.full(grid.shape, np.nan), where=denom > 1e-12)

        return grid, smooth

    def smooth_expression_matrix_by_pseudotime(pt, expr_mat, sigma_pt=BIO_PT_SIGMA, grid_size=BIO_GRID_SIZE):
        pt = np.asarray(pt, dtype=float)
        expr_mat = np.asarray(expr_mat, dtype=float)

        valid = np.isfinite(pt) & (pt >= 0)
        pt, expr_mat = np.clip(pt[valid], 0, 1), expr_mat[valid]

        if len(pt) < 3 or np.ptp(pt) < 1e-8:
            return None, None, expr_mat

        lo, hi = pt.min(), pt.max()
        grid = np.linspace(lo, hi, grid_size)
        bin_idx = np.rint((pt - lo) / (hi - lo) * (grid_size - 1)).astype(int)
        bin_idx = np.clip(bin_idx, 0, grid_size - 1)

        n_genes = expr_mat.shape[1]
        sum_grid = np.zeros((grid_size, n_genes))
        count_grid = np.zeros((grid_size, n_genes))

        for b in np.unique(bin_idx):
            block = expr_mat[bin_idx == b]
            finite = np.isfinite(block)
            sum_grid[b] = np.nansum(block, axis=0)
            count_grid[b] = finite.sum(axis=0)

        delta = (hi - lo) / max(grid_size - 1, 1)
        sigma_bins = max(0.5, sigma_pt / max(delta, 1e-8))

        smooth_sum = gaussian_filter1d(sum_grid, sigma=sigma_bins, axis=0, mode='constant')
        smooth_count = gaussian_filter1d(count_grid, sigma=sigma_bins, axis=0, mode='constant')

        smoothed = np.divide(
            smooth_sum, smooth_count,
            out=np.full_like(smooth_sum, np.nan),
            where=smooth_count > 1e-12
        )

        return grid, smoothed, expr_mat
    ...
    def prepare_spline_heatmap(path_cells, marker_genes=(), top_n=DYNAMIC_TOP_N):
        ...
        Test expression ~ natural spline(pseudotime, df=5) against an intercept-only model.
        Select significant dynamic genes by BH-FDR, then rank by R^2 and keep at most Top-N.
        """
        ...
        sd = np.sqrt(np.maximum(np.asarray(X.multiply(X).mean(axis=0)).ravel() - mean**2, 0))
        eligible = np.where(
            np.isfinite(sd) & (sd >= DYNAMIC_MIN_SD) & (det >= DYNAMIC_MIN_DETECTION)
        )[0]
        if len(eligible) < 1:
            return None
        ...
        r2 = np.divide(gain, sse0, out=np.zeros_like(gain), where=sse0 > 1e-12)
        mse1 = sse1 / df_den
        F = np.divide(gain / df_num, mse1, out=np.zeros_like(gain), where=mse1 > 1e-12)
        p = f_dist.sf(F, df_num, df_den)
        q = bh_fdr(p)

        sig_local = np.where(np.isfinite(q) & (q <= DYNAMIC_FDR_REPORT))[0]
        if len(sig_local) == 0:
            return None

        sig_rank = sig_local[np.argsort(-r2[sig_local], kind="mergesort")]
        ...
        Dg = np.asarray(
            build_design_matrices([design_df.design_info], {"pt": grid})[0],
            float
        )
        curves = Dg @ beta[:, chosen_local]

        lo, hi = np.nanmin(curves, axis=0), np.nanmax(curves, axis=0)
        scaled = (curves - lo) / (hi - lo + 1e-9)
        order = np.argsort(grid[np.nanargmax(scaled, axis=0)], kind="mergesort")
    ...
    for t_name in TARGET_LINEAGES:
        matches = [k for k in dijkstra_paths if str(cell_types_arr[k]) == t_name]
        if matches:
    ...
    type_to_node = {str(cell_types_arr[node]): node for node in dijkstra_paths}
    selected_ends_ef = [type_to_node[t] for t in TARGET_LINEAGES if t in type_to_node]
    ...
    ax_a.text(origin_x + 0.09, origin_y - 0.03, "UMAP1", transform=ax_a.transAxes, ha='center', va='top')
    ax_a.text(origin_x - 0.03, origin_y + 0.09, "UMAP2", transform=ax_a.transAxes,
              ha='right', va='center', rotation=90)

    # ============================================================
    # B. Trajectory
    # ============================================================
    ax_b.scatter(umap_coords[:, 0], umap_coords[:, 1], c='#e0e0e0',
                 alpha=0.4, s=15, edgecolors='none')

    ax_b.scatter(umap_coords[start_idx, 0], umap_coords[start_idx, 1],
                 fc=START_COLOR, ec=START_EDGE, marker=START_MARKER,
                 s=START_SIZE, zorder=15)

    for seg, nodes in zip(GLOBAL_SMOOTH_SEGS, GLOBAL_NODE_LISTS):
        belonging = [e for e, p in all_target_paths.items() if all(n in p for n in nodes)]
        labels = [str(cell_types_arr[e]) for e in belonging if str(cell_types_arr[e]) in TARGET_LINEAGES]

        if not labels:
            continue

        line_color = EXPLICIT_COLORS[TARGET_LINEAGES[0]] if len(labels) > 1 else EXPLICIT_COLORS[labels[0]]
        ax_b.plot(seg[:, 0], seg[:, 1], color=line_color, lw=3.5, alpha=0.85, zorder=10)
    ...
    for end_idx in ends_idx:
        ...
        if label in TARGET_LINEAGES:
            ax_b.scatter(umap_coords[end_idx, 0], umap_coords[end_idx, 1],
                         c=EXPLICIT_COLORS[label], marker=END_MARKER,
                         s=END_SIZE, edgecolors='white', zorder=12)
    ...
    sc_c = ax_c.scatter(umap_coords[:, 0], umap_coords[:, 1],
                        c=pt_norm, cmap='viridis', s=15, alpha=0.9, edgecolors='none')
    ...
    ax_c.scatter(umap_coords[start_idx, 0], umap_coords[start_idx, 1],
    ...
    for end_node, raw_rewards in path_steps_dict.items():
        ...
        ax_d.plot(range(len(raw_rewards)), raw_rewards, marker='o', ms=4, lw=2.5,
                  color=EXPLICIT_COLORS[label], alpha=0.85, label=label)
    ...
    # ============================================================
    # E. Marker dynamics
    # Each panel shows one gene's smoothed expression curve
    # Corresponds to G panel (scheme B)
    # ============================================================
    target_genes = ['FCGR3A', 'CLEC10A']

    for idx, (gene, ax) in enumerate(zip(target_genes, axes_e)):
        ...
        for end_node in selected_ends_ef:
            ...
            ax.plot(grid, curve, color=EXPLICIT_COLORS[end_type],
                    lw=3.0, alpha=0.95, label=f"Trajectory {end_type}")
        ...
        ax.annotate('', xy=(1.04, 0), xycoords='axes fraction', xytext=(0, 0),
                    arrowprops=dict(arrowstyle='->', color='black', lw=1.5),
                    annotation_clip=False)
        ax.annotate('', xy=(0, 1.04), xycoords='axes fraction', xytext=(0, 0),
                    arrowprops=dict(arrowstyle='->', color='black', lw=1.5),
                    annotation_clip=False)

        ax.set_xlabel("Global trajectory pseudotime", fontsize=12, fontweight='bold', labelpad=12)

        if idx == 0:
            ax.set_ylabel("Smoothed normalized expression", fontsize=12, fontweight='bold', labelpad=16)

    # ============================================================
    # G. Marker feature plots
    # Corresponding to E panel (scheme B)
    # ============================================================
    ...
    for (gene, target_type), ax in zip(target_pairs, axes_g):
        ...
        sc_plot = ax.scatter(umap_coords[order, 0], umap_coords[order, 1],
                             c=expr_all[order], cmap='plasma',
        ...
        line_color = EXPLICIT_COLORS[target_type]
        ax.plot(path_coords[:, 0], path_coords[:, 1], color=line_color, lw=4, zorder=10)
        ax.scatter(path_coords[0, 0], path_coords[0, 1], c='white', ec='black',
                   marker='*', s=200, zorder=15)
        ax.scatter(path_coords[-1, 0], path_coords[-1, 1], c=line_color, ec='white',
                   marker='X', s=100, zorder=15)

        ax.set_title(f"{gene} Expression\n→ Trajectory {target_type}", fontsize=15)
        ax.axis('off')

        cbar = fig.colorbar(sc_plot, ax=ax, orientation='horizontal', fraction=0.04, pad=0.08)
        cbar.set_ticks([expr_all.min(), expr_all.max()])
        cbar.set_ticklabels(['Low', 'High'])

    # ============================================================
    # F. Natural-spline pseudotime-associated genes on each inferred trajectory path
    # ============================================================
    CLUSTER_MARKERS = {
        '0': [
            'CD14', 'S100A8', 'S100A9', 'S100A12',
            'FCN1', 'VCAN', 'CCR2', 'SELL'
        ],
        '1': [
            'CD14', 'FCGR3A', 'HLA-DRA', 'CD74',
            'HLA-DPB1', 'CCR5', 'CD40'
        ],
        '2': [
            'FCGR3A', 'MS4A7', 'CX3CR1', 'RHOC',
            'CDKN1C', 'TCF7L2', 'HES4', 'IFITM3'
        ],
        '3': [
            'CD1C', 'CLEC10A', 'FCER1A', 'CD1E',
            'HLA-DRA', 'HLA-DPA1', 'HLA-DPB1', 'CD74'
        ],
    }

    marker_map = {
        '2': CLUSTER_MARKERS['0'] + CLUSTER_MARKERS['1'] + CLUSTER_MARKERS['2'],
        '3': CLUSTER_MARKERS['0'] + CLUSTER_MARKERS['1'] + CLUSTER_MARKERS['3'],
    }
    ...
    for idx, (end_node, ax) in enumerate(zip(selected_ends_f, axes_f)):
        end_type = str(cell_types_arr[end_node])
        path_cells = np.asarray(dijkstra_paths[end_node], dtype=int)
        heat = prepare_spline_heatmap(path_cells, marker_map.get(end_type, []), DYNAMIC_TOP_N)

        if heat is None:
            ax.set_axis_off()
            print(f"Trajectory {end_type}: path_cells={len(path_cells)}, spline analysis unavailable")
            continue

        grid, genes = heat["grid"], np.asarray(heat["gene_names"], str)
        im = ax.imshow(heat["scaled"].T, aspect="auto", cmap="viridis", interpolation="nearest",
                       vmin=0, vmax=1, extent=[grid[0], grid[-1], len(genes), 0])
        ax.set_title(f"→ Trajectory {end_type}", fontsize=16, pad=15, color="black", fontweight="bold")
        ax.set_yticks([])
        ax.set_xticks([])
        ax.set_xlabel("Trajectory pseudotime →", fontsize=13, fontweight="bold")
        ax.set_ylabel(f"Dynamic genes (n={len(genes)})", fontsize=14)

        for gene in heat["markers_in_topn"]:
            y = np.where(genes == gene)[0][0] + 0.5
            ax.annotate(
                gene, xy=(grid[-1], y), xycoords="data", xytext=(1.10, y),
                textcoords=("axes fraction", "data"),
                arrowprops=dict(facecolor="red", edgecolor="red", width=3.5, headwidth=9, headlength=8),
        ...
        print(f"Trajectory {end_type}: path_cells={len(path_cells)}, "
              f"eligible_genes={heat['n_eligible']}, q<{DYNAMIC_FDR_REPORT:g}={heat['n_significant']}, "
              f"plotted={len(genes)}")
    ...
    for label, ax in zip(
        ['a', 'b', 'c', 'd', 'e', 'g', 'f'],
        [ax_a, ax_b, ax_c, ax_d, ax_e1, ax_g1, ax_f1]
    ):
